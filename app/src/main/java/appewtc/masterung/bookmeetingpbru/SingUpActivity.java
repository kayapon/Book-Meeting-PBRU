...
public class SingUpActivity extends AppCompatActivity {
    ...
    public void clickSignUpSign(View view) {
        ...
        if (checkSpace()) {
            ...
            //have this User
            MyAlert myAlert = new MyAlert();
            myAlert.myDialog(this, "มี User นี้แล้ว", "กรุณาเปลี่ยน");

        }else if (checkIDcard(idCardString)){
            //Have thid idCard
    ...
    private boolean checkIDcard(String idCardString) {

        try {
            SQLiteDatabase sqLiteDatabase = openOrCreateDatabase(MyOpenHelper.database_name,
                    MODE_PRIVATE, null);
            Cursor cursor = sqLiteDatabase.rawQuery("SELECT * FROM userTABLE WHEWE IDcard = " + "'" + idCardString + "'", null);
            cursor.moveToFirst();
